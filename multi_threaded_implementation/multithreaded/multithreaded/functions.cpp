...
void ZNCC(vector< vector <unsigned char> >&sample1, vector< vector <unsigned char> >&sample2, vector<vector<zncc_parameters>> &zncc1, vector<vector<zncc_parameters>> &zncc2, vector<vector<zncc_parameters>> &zncc3)
{
	...
	for (auto& th : threads)
		th.join();
	*/
	//uncomment of you want to make 252 threads
	vector<thread> threads;
	for (int y = 0; y < HEIGHT/2; y++)
	{
		threads.push_back(thread(processing_per_line, ref(sample1), ref(sample2), ref(zncc1[y]), ref(zncc2[y]), ref(zncc3[y]), y));
		threads.push_back(thread(processing_per_line, ref(sample1), ref(sample2), ref(zncc1[y + HEIGHT / 2]), ref(zncc2[y + HEIGHT / 2]), ref(zncc3[y + HEIGHT / 2]), y+HEIGHT/2));
